  ...
  return (
    <div className="h-80 pt-5 bg-white">
      <div className="grid grid-cols-3">
        {data.map((val, i) => {
          return (
            <div key={i} className="h-60 border-b-4 border-b-transparent border-r-2 px-6 pb-3 hover:border-b-4 hover:border-b-mainColor delay-100 duration-700 transition-all">
                <div className="flex items-center gap-2 font-semibold text-lg pb-2 ">
                    <val.image className="text-mainColor font-bold" />
                  <p>{val.title}</p>
                </div>
                <p className="text-sm text-gray-600">{val.info}</p>
              </div>
          )
        })}
      </div>
